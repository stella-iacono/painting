...
let balloonSize = 100;
let windSpeed = 0; // Horizontal movement speed
let popped = false;

function setup() {
    createCanvas(550, 560);
    balloonX = width / 2;
    balloonY = height / 2 + 100;
}
...
function draw() {
    ...
    if (dist(mouseX, mouseY, balloonX, 100) < balloonSize / 2) {
        windSpeed = (mouseX - balloonX) * 0.05;
    } else {
        windSpeed = 0; // No wind effect when not hovering
    }
    ...
    if (!popped) {
        fill(200, 50, 50);
        circle(balloonX, 100, balloonSize, balloonSize * 1.2);
      } else {
        fill(0);
        textSize(32);
    ...
    // Check if mouse is over the balloon and clicked
    if (dist(mouseX, mouseY, balloonX, 100) < balloonSize / 2 && !popped) {
      popped = true;
    }
}
